...
# Paths
model_path = "resnet101_human_classifier.pth"
class_map_path = "human_class_to_idx.pkl"
    ...
    for poly_file in files:
        ...
        for key, polygon_points in all_polygon_points.items():
            for poly in polygon_points:
                ...
                classifier_response = predict_image_label(cropped_image)
                gt = None
                if 'human' in key:
                    gt_list = key.split('--')
                    if len(gt_list)==3:
                        ...
                        gt = key
                if gt and gt != 'person-group' and classifier_response:
                    response11 = classifier_response.lower()
